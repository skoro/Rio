...
type
  ...
  { :: INFO :: INFO :: INFO ::
    This parser almost the same as in jsonparser unit.
    The difference is only of using a custom class for formatting
    float values instead of 3.200000E+1.
    I didn't manage to find a correct way to override ParseNumber method of
    TJSONParser class only as just copy-paste the original parser class
    with the some patch for a float number.
  }
  ...
  TJSONFormattedFloat = class(TJSONFloatNumber)
  protected
...
function TJSONFormattedFloat.GetAsString: TJSONStringType;
begin
  Result := GetAsFloat.ToString;
end;
{ TJSONParser }
...
begin
  ...
  if TryStrToQWord(S,QW) then
    ...
    If TryStrToInt64(S,I64) then
      ...
      Result:=TJSONFormattedFloat.Create(F);
      end;
    end;
